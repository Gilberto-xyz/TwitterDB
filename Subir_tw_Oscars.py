...
dataframe = pd.read_csv('Oscars.csv')
...
cursor.execute('CREATE TABLE tweets_Oscars (id INT AUTO_INCREMENT PRIMARY KEY, Date VARCHAR(255), Username VARCHAR(255), Tweet VARCHAR(255), Polarity VARCHAR(255), Subjectivity VARCHAR(255))')

# Insertar valores del dataframe en la tabla creada en la base de datos MySQL 
for i, row in dataframe.iterrows():
    sql = "INSERT INTO tweets_Oscars (Date, Username, Tweet, Polarity, Subjectivity) VALUES (%s, %s, %s, %s, %s)"
